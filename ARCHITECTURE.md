...
- **ffmpeg-static** — FFmpeg 预编译二进制（音频响度标准化）
...
- `title`: 标题
- `description`: 描述
- `coverUrl`: 封面图 URL（OSS，图文投稿时使用用户选择的封面或第一张图片）
- `videoUrl`: 视频 URL（OSS，图文投稿时为空）
...
- `id`: 主键 (CUID)
- `content`: 评论内容
- `images`: 图片 URL 列表（JSON 字符串，最多 7 张，可选）
- `videoId`: 视频 ID
- `authorId`: 评论者 ID
...
- `GET /api/videos/[videoId]/comments` — 获取评论列表（三层嵌套树形结构，含 images 字段）
- `POST /api/videos/[videoId]/comments` — 发表评论/回复（支持 parentId 嵌套、images 图片数组，文字或图片至少一项）
...
- **密码明文存储**：MVP 阶段密码以明文存储和比较。生产环境必须使用 `bcryptjs`（已安装但未启用）进行哈希存储
...
- **MVP 说明**：CSRF 保护覆盖范围是 MVP 阶段的有意设计。完整生产环境应为所有写操作端点添加 CSRF 校验
...
- 新登录时从数据库同步 `tokenVersion`（避免硬编码为 0 导致改密码后新 token 立即失效）
...
### 文件上传安全
- 视频文件大小限制：500MB
- 图片文件大小限制：15MB（投稿图片），5MB（封面）
- 音乐文件大小限制：50MB
- 扩展名白名单：mp4/webm/mov/avi/flv/mkv（视频）、jpg/jpeg/png/gif/webp/bmp（图片）、mp3/wav/ogg/aac/flac（音乐）
- 文件名由服务端生成（`timestamp_random.ext`），不使用原始文件名
- 客户端压缩：图片超过15MB或分辨率超过2K时自动压缩（Canvas API，保守目标90%）
- 压缩确认：大文件压缩前弹窗确认，预估压缩后大小
...
3. **配置 Nginx（HTTPS 反向代理）**
   ...
   证书文件放在 `/etc/nginx/ssl/`：
   - `your-domain.com.pem` — 全链证书（DigiCert）
   - `your-domain.com.key` — RSA 私钥

   配置文件 `/etc/nginx/conf.d/ssl-your-domain.conf`：
   ```nginx
   ...
   > **限流说明**：Next.js 页面刷新会并发发出大量 RSC 请求（首页 + 多个路由），
   > rate 从 10r/s 提升到 50r/s，burst 从 20 提升到 100，避免刷新时 503。

   > **证书限制**：当前证书仅覆盖 `your-domain.com` + `www.your-domain.com`，
   > 如需其他子域名 HTTPS，需更换为 `*.your-domain.com` 通配符证书。
...
- `tailwindcss@4.1.4` (dev) — CSS 框架（内置 `backdrop-blur-*` 毛玻璃工具类）
...
16. **点击自动播放** — 从首页/个人中心/推荐列表点击视频封面进入播放页时，视频自动非静音播放（利用浏览器用户交互策略）
17. **全页面移动端适配** — 所有页面使用 sm: 断点（640px）做移动端/桌面端区分，响应式布局
18. **移动端双击暂停** — 移动端通过 touchend 检测双击（300ms 间隔）暂停/播放视频，桌面端保持单击
19. **Header 移动端搜索动画** — 移动端点击搜索图标展开全宽搜索栏（绝对定位覆盖），收起时淡出
20. **移动端头像菜单** — 移动端点击头像切换菜单开关，桌面端保持 hover 行为
21. **公共用户主页** — `/user/[id]` 显示其他用户的公开信息（投稿数/获赞/收藏）和投稿视频列表
22. **视频播放页作者信息** — 显示作者头像+名字，点击跳转公共主页
23. **投稿页移动端提示** — 移动端显示"投稿功能仅支持PC端"提示，桌面端保持投稿表单
24. **Framer Motion 动画** — 视频卡片 staggered 入场、点赞/收藏按钮弹跳、评论滑入塌陷、推荐列表滑入、头像菜单淡入缩放、视频播放页入场、深色模式过渡动画
25. **个人主页标签动画** — 标签下划线 spring 滑动、标签内容 AnimatePresence 左右滑入滑出、hashchange 监听菜单切换
26. **登录/注册弹窗** — Header 点击登录/注册按钮弹出悬浮弹窗，Tab 切换登录和注册模式，注册成功自动将用户名带入登录表单
27. **用户注销功能** — 个人主页设置标签危险操作区域，自定义确认弹窗（替代浏览器 confirm），输入密码确认注销，后端递归删除所有用户数据
28. **收藏计数显示** — 收藏按钮显示收藏数量，服务端预查询初始状态，实时更新；视频卡片同时显示点赞数和收藏数
29. **播放器控制栏优化** — 上一个/下一个使用 lucide-react 图标，控制栏重新排列（上一个→播放→下一个）
30. **登录/注册弹窗** — Header 点击登录/注册按钮弹出悬浮弹窗，Tab 切换登录和注册模式，注册成功自动将用户名带入登录表单
...
- **评论输入框** — 弹性延展设计，最多10行，超过显示滚动条（深色/浅色模式适配）
...
- **音频标准化优先**：当 `audioNormalized=true` 且 `normalizedUrl` 存在时，优先使用 `normalizedUrl` 作为 `source` 播放，跳过 VOD 鉴权；仅当 `normalizedUrl` 不存在时才走 `vid + playauth` 鉴权路径
- OSS 视频：通过 `source` 直链播放
...
- 跳转使用类型安全的 sessionStorage 信号工具（`src/lib/signals.ts`）存储目标评论 ID（`setHighlightComment`/`consumeHighlightComment`），避免 Next.js 客户端导航丢失 URL hash
- 评论区组件读取信号后，轮询等待目标评论 DOM 渲染完成
...
- 图片路径：`images/{时间戳}_{随机字符串}.{扩展名}`（图文投稿、评论图片）
- 音乐路径：`music/{时间戳}_{随机字符串}.{扩展名}`（图文投稿）
- 删除视频时同步删除 OSS 文件
- 投稿页文件选择器使用自定义按钮，粘贴/拖拽后正确显示文件名
- 评论图片上传：复用 `/api/upload` 接口（type=image），前端并行上传多张图片
...
### 音频响度标准化
- **目标**：统一所有视频的音频响度，提升观看体验（避免有的视频声音大、有的小）
- **标准**：EBU R128，目标响度 -24 LUFS，True Peak 限制 -2 dBTP，响度范围 7 LU
- **精度**：两次通过（two-pass）loudnorm，精度 ±0.5 LU（单次通过仅 ±1~3 LU）
- **处理范围**：所有视频类型（video + image_text），video 用 videoUrl/vodVideoId，image_text 用 musicUrl
- **处理方式**：异步队列，上传后自动处理，不阻塞播放
- **核心文件**：
  - `src/lib/audio-normalize.ts` — FFmpeg loudnorm 滤镜封装，使用 `ffmpeg-static` 预编译二进制，两次通过处理
  - `src/lib/audio-queue.ts` — 内存队列，并发处理（最多 2 个任务同时运行），每 5 秒检查一次，失败最多重试 2 次，详细错误日志
  - `src/lib/vod.ts` — 新增 `getVodPlayUrl()` 函数，调用 VOD `GetPlayInfo` API 获取带签名的播放 URL
- **处理流程**：
  1. 用户上传视频 → 创建视频记录 → 调用 `addToQueue(videoId)` 立即入队（video 用 videoUrl/vodVideoId，image_text 用 musicUrl）
  2. Worker 从队列取出任务 → 获取音频下载 URL（videoUrl、VOD 播放 URL 或 musicUrl）→ FFmpeg 两次通过标准化音频（第一遍分析统计信息，第二遍精确调整）→ 上传标准化版本到 OSS（video 存 `videos/`，image_text 存 `music/`）→ 更新数据库
  3. 用户播放视频 → 检查 `audioNormalized` → 使用 `normalizedUrl`（已标准化）或原始 URL（未标准化）；图文播放器 ImageCarousel 优先使用 normalizedUrl 播放标准化后的音乐
- **两次通过处理**：
  - 第一遍：`loudnorm=I=-24:TP=-2:LRA=7:print_format=json` 分析音频统计信息（measured_I、measured_TP、measured_LRA、measured_thresh）
  - 第二遍：使用测量值精确调整 `loudnorm=...:measured_I=...:measured_TP=...:measured_LRA=...:measured_thresh=...:linear=true`
- **流式处理**：`normalizeAudioFromUrl()` 支持从远程 URL 流式下载到临时文件，FFmpeg 直接处理临时文件，避免全量文件加载到内存
- **视频来源支持**：
  - OSS 直链视频：直接通过 HTTP 流式下载
  - VOD 鉴权视频：调用 `GetPlayInfo` API 获取带签名的播放 URL（有时效性）
- **启动时回填**：服务器启动时自动扫描数据库里所有 `audioNormalized=false` 且有 `videoUrl`、`vodVideoId` 或 `musicUrl` 的记录，加入队列处理
- **失败日志**：处理失败时记录视频 ID、尝试次数（attempt N/M）、错误原因；重试耗尽时记录 "GIVING UP" 消息
- **FFmpeg 路径**：生产环境下 Next.js 打包会导致 `ffmpeg-static` 的 `__dirname` 路径错误，使用 `require.resolve` 手动定位二进制文件

### 性能优化
- **搜索 API 优化** — 搜索从全表加载+内存过滤改为数据库 `LIKE` 预过滤+内存高亮定位，大幅减少内存占用和响应时间
- **评论删除 BFS** — 递归删除子评论从 N+1 递归查询改为迭代式 BFS 批量查询，每层子评论只需 1 次 DB 查询
- **音频队列并发** — 音频标准化队列从串行（1 个任务）改为并发（最多 2 个任务并行处理），吞吐量提升 2x
- **FFmpeg 流式处理** — 新增 `normalizeAudioFromUrl()` 函数，FFmpeg 直接处理远程 URL 流式下载到临时文件，避免全量视频加载到内存
- **上传路由流式** — 大文件（>5MB）使用 `Readable.fromWeb()` + `oss.putStream()` 流式上传，避免全量 Buffer 拷贝
- **VideoCard useMemo** — 视频卡片头像颜色计算使用 `useMemo` 缓存，避免列表重渲染时重复计算
- **VOD 缓存 LRU** — `playAuthCache` 添加最大 100 条限制和过期淘汰机制，防止长时间运行内存泄漏
- **数据库索引** — Video/Comment/Like/Favorite 表添加常用查询字段索引（authorId、createdAt、videoId、parentId、userId、title）
