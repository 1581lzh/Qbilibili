...
- 图文播放器左右切换按钮不误触发暂停
- 图文播放器相册式左右平移（当前页+前后相邻页三页轨道并排常驻，鼠标左键按住/触摸拖动实时跟手，松手超1/4页宽滑入相邻页（头尾循环）否则回弹；进度条/左右按钮/方向键/A-D/自动轮播统一滑动动画；每页独立高斯模糊背景随图平移不串色）
- 图文播放器预加载窗口（预览当前图时提前加载前后各2张图片及其实况视频数据，切换即就绪；实况非预览时只显示封面不播放）
- 图文播放器底部图片进度条（始终显示，每张图片一段，播放时白色逐渐填充，暂停就地冻结，手动切换满白，恢复重播归零，点击进度条某一节跳转到对应图片，悬停加粗3px→9px中线对称扩展80ms过渡；控制栏 0fr↔1fr 水托荷叶动画升起/落下，进度条被托起或回落，无重复进度条）
- 图文播放器音量控制（控制栏音量按钮，点击静音/取消静音，悬停弹出滑块调节背景音乐音量，滑块显示粉色填充进度，thumb 圆心与轨道中线对齐；音量条悬停弹出 2s 自动消失，滑入滑块保持显示；音量/静音状态按用户存数据库 User.volume/muted，未登录 fallback 本地存储）
...
- 图文播放器中心蒙版动画（播放/暂停弹性图标 z-40 浮于主图/控制栏之上）
- 图文播放器控制栏空白点击只切换控制栏可见性，不触发播放/暂停
- 图文播放器左右切换按钮 z-30 高于主图 z-10 保证可点击，hover 蒙版加深 40%
- 图文播放器移动端进度条（单击显示/隐藏控制栏，默认显示3秒后自动隐藏，隐藏导航按钮）
- 图文播放器多首音频拼接（按顺序播放多首音频，所有音频总时长用于自动计算图片停留时长）
- 图文图片预览时长支持自动模式（根据音频总时长和图片数量自动计算）和手动模式（1-30秒）
- 实况照片（Live Photo）支持（Android Motion Photo 单文件解析 / Apple .livp 解包 / HEIC 分离格式同名配对，播放时自动静音播放实况视频，进度条显示视频进度，视频完整播完再切下一张，静态图时长 = (总音频-实况总时长)/静态图数 拉平节奏）
- 高斯模糊背景填充黑边（视频用封面图、图文用当前图做模糊背景，比较媒体实际比例与容器比例自适应移动端/PC端，图文每页模糊随图平移、overflow-hidden 防串色）
- 上传视频（支持拖拽、粘贴、进度条）
...
- 登录用户按用户存数据库（User.theme，跨设备/刷新恢复），未登录 fallback localStorage
- localStorage 键沿用 `theme`，刷新不丢失
- 页面加载前内联脚本防白闪
- 圆形遮罩扩散动画（点击按钮处生成纯色圆，z-index:-1 背景层只遮空白不挡UI，恒定速度扩散；连续切换圆中圆叠加，最上层定最终色；背景扩散完成后文字/UI 单独渐变）
...
│   │   ├── live-photo.ts            # 实况照片识别/解析（motion-photo 单文件 + .livp 解包 + HEIC 转码 + 同名配对）
│   │   ├── emoji.ts                 # Emoji 工具函数（Unicode emoji 解析 + 抖音表情包匹配）
│   │   ├── emoji-data.ts            # Unicode emoji 分类数据（按类别组织的 emoji 列表）
│   │   ├── avatar.ts                # 头像颜色共享模块（avatarColorFor()，按用户名哈希统一头像色）
│   │   ├── keyboard.ts              # 键盘控制守卫（isEditableTarget/isComposingEvent，输入态解除快捷键）
...
- musicUrls: 背景音乐 URL 列表（JSON 字符串，多首音频拼接）
- imageDuration: 图片轮播时长（秒，图文类型，null=自动）
