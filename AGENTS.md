...
- 图文播放器左右切换按钮不误触发暂停
- 图文播放器底部图片进度条（每张图片一段，播放时白色逐渐填充，暂停就地冻结，手动切换满白，恢复重播归零，点击进度条某一节跳转到对应图片，悬停加粗3px→9px中线对称扩展80ms过渡）
...
- 高斯模糊背景填充黑边（视频用封面图、图文用当前图做模糊背景，比较媒体实际比例与容器比例自适应移动端/PC端）
- 图文背景切换交叉淡化（AnimatePresence 让背景随内容一起切换，非原地突变）
- 上传视频（支持拖拽、粘贴、进度条）
