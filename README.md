...
- 图文播放器缩放（纯前端中心缩放，不跟随鼠标指针：鼠标滚轮/双击缩放、触摸双指捏合缩放、右上角 +/−/重置按钮、键盘 +/−/0 缩放；scale>1 时拖动=平移图片、切页箭头隐藏，scale≈1 恢复相册拖拽切页；缩放期间暂停自动轮播、退出缩放继续；切页/进度条跳转自动归位；实况照片同步缩放，canvas 绘制分辨率跟随 scale×dpr 保持放大清晰；实况 video+canvas 移入当前页 slot 随轨道/缩放对齐，单图实况只渲染一份）
