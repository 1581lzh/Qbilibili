...
- 点击视频封面自动非静音播放
- 播放/暂停中心弹性动画
- 内容描述折叠（播放页描述超过 1.5 行时折叠：第一行完整 + 第二行按字符量在"···"处截断（非垂直腰斩），截断点后同一行显示"展开"按钮，展开后末尾"折叠"按钮；视频和图文类型共用）
- 上传视频到阿里云 VOD（自动回退 OSS）
- 投稿页表单布局（视频/图文投稿左侧预览窗口与右侧表单等高对齐；上传素材后预览左固定 3:4 比例顶部对齐，不被右侧内容拉长；视频投稿表单顺序：标题→简介→视频文件→封面图，与图文投稿一致）
...
- 图文播放（图片轮播+多首音频拼接+音频自动播放+空格键暂停+相册式左右平移切换（鼠标左键按住拖动/触摸滑动/方向键与A/D键/左右按钮/进度条跳转，头尾循环）+自动轮播+播放模式切换（循环/单次/自动连播，与视频共享）+底部图片进度条（始终显示，点击跳转+悬停加粗，控制栏升起时被托起、落下时回到底部）+音量控制（静音+滑块，音量条/模式提示弹层不被控制栏裁切，调节音量不误触播放/暂停、切页、缩放）+弹性动画指示器+PC单击暂停+移动端双击暂停+预加载窗口（当前图前后各 2 张图片及其实况视频数据提前加载））
- 图文图片预览时长支持自动模式（根据音频总时长和图片数量自动计算每张图片停留时长）和手动模式（1-30秒，图文投稿默认手动 3 秒）
...
- 图文播放器缩放（纯前端中心缩放，不跟随鼠标指针：鼠标滚轮/双击缩放（双击仅在图片/空白区域生效，音量/模式/全屏等控件上不触发）、触摸双指捏合缩放、右上角 +/−/重置按钮、键盘 +/−/0 缩放；scale>1 时拖动=平移图片、切页箭头隐藏，scale≈1 恢复相册拖拽切页；缩放期间暂停自动轮播、退出缩放继续；切页/进度条跳转自动归位；实况照片同步缩放，canvas 绘制分辨率跟随 scale×dpr 保持放大清晰；实况 video+canvas 移入当前页 slot 随轨道/缩放对齐，单图实况只渲染一份）
- 图文播放器移动端与全屏体验优化（横屏播放器高度自适应防止溢出；滑动切换阈值 1/4 页宽→1/5 页宽更灵敏；全屏下进度条左右收窄避开手机四角圆弧（只缩进度条，控制栏保持全宽）；全屏进出自动复位轨道偏移/过渡/缩放状态，修复切页动画中进全屏导致内容错位；移动端系统返回键：缩放中先重置缩放停留页内、再按才退出，全屏+缩放时退出全屏瞬间重置缩放并重进全屏（浏览器拒绝时优雅降级）；手动切到实况图不置 userInteracted，切过去即预览→淡入播放，静态图仍暂停轮播；实况 canvas 绘制像素总预算 400 万，超预算等比压缩分辨率保证全屏+高 DPR+高缩放下流畅）
- 图文投稿实况视频（可上传 ≤20 秒短视频作为实况，超时提示前往视频投稿；大预览/缩略图播放预览，播完自动暂停）
...
- 评论图片附件（每条最多7张，文字可选；单图等比缩小显示，多图 1:1 正方形缩略图网格（移动端 3 列 / PC 5 列，超出显示黑底白字 +N 小标），GIF 动图完整显示；全屏灯箱查看，支持文件选择/粘贴上传/粘贴图片URL自动下载，自动压缩，GIF 超限提示不压缩保动画）
- 评论图片灯箱（相册式三页轨道滑动切换：移动端单指滑动/桌面端鼠标拖拽实时跟手，松手超 1/4 页宽滑入相邻页否则回弹，头尾循环；缩放 0.5-5 倍：鼠标滚轮/按钮/键盘缩放 + 移动端双指增量捏合，双击切换缩放（鼠标原生 dblclick 判定 250%/100% 切换、触摸原生状态机判定，互不重复触发），拖拽平移，缩放下限 0.5 倍；单击图片不会误关灯箱；移动端系统返回键直接关闭灯箱）
...
- Emoji 显示/输入统一（标题、简介、搜索等所有纯文本位置统一渲染 `[code]` 为 emoji 图形；标题/简介/编辑页 contentEditable 输入组件实时显示、提交时还原为 `[code]` 文本存储，支持 maxLength 截断与 IME 防抖；214 个抖音表情拆成两张雪碧图按需加载）
- 收藏按钮显示收藏数量
...
- 音量状态持久化（图文播放器和视频播放器音量/静音状态共享，按用户存数据库，未登录 fallback 本地存储；图文音量条悬停弹出 1.5s 自动消失；视频播放器启动即恢复保存的音量与静音设置，避免首帧以最大音量播放）
...
> **构建提示**：服务器内存仅 3.6G，编译构建（`npm run build`）前需先 `sudo systemctl stop bilibili.service` 停止服务释放内存，再执行 `export NODE_OPTIONS=--max-old-space-size=1536 && npm run build`（package.json 已内置该内存限制），构建完成后 `sudo systemctl start bilibili.service` 重启服务。
