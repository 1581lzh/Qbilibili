...
- 图文播放（图片轮播+多首音频拼接+音频自动播放+空格键暂停+滑动切换+自动轮播+播放模式切换（循环/单次/自动连播，与视频共享）+底部图片进度条（点击跳转+悬停加粗）+音量控制（静音+滑块）+弹性动画指示器+PC单击暂停+移动端双击暂停）
- 图文图片预览时长支持自动模式（根据音频总时长和图片数量自动计算每张图片停留时长）和手动模式（1-30秒）
- 实况照片（Live Photo）支持（Android Motion Photo 单文件解析 / Apple .livp 解包 / HEIC 分离格式同名配对；播放时自动静音播放实况视频，进度条显示视频进度，视频完整播完再切下一张；静态图时长 = (总音频-实况总时长)/静态图数 拉平节奏；实况只能用静态帧做封面）
- 高斯模糊背景填充黑边（视频用封面图、图文用当前图做模糊背景，比较媒体实际比例与容器比例自适应移动端/PC端，背景随内容交叉淡化切换）
