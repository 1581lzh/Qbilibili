                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                  {uploads.map((v) => (
                    <div key={v.id} className="group block">
                       <Link href={`/video/${v.id}`} onClick={() => setAutoPlayVideo(v.id)}>
                        <div className="overflow-hidden rounded-lg bg-zinc-200 dark:bg-zinc-800">
                          <img
                            src={optimizedCover(v.coverUrl)}
                            alt={v.title}
                      ...
                      <div className="space-y-4">
                        {likedComments.map((comment) => (
                          <a
                            ...
                            onClick={(e) => { e.preventDefault(); setHighlightComment(comment.id); window.location.href = `/video/${comment.video.id}`; }}
                      ...
                      <div className="space-y-4">
                        {receivedLikes.map((item) => (
                          <a
                            ...
                            onClick={(e) => { e.preventDefault(); setHighlightComment(item.comment.id); window.location.href = `/video/${item.comment.video.id}`; }}
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {comments.map((comment) => (
                    <div
                      key={comment.id}
                      onClick={() => { setHighlightComment(comment.id); window.location.href = `/video/${comment.video.id}`; }}
